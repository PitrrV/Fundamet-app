...
const { data, error } = await supabase
  ...
  .select("currency_code, narrative, scenarios")
  .order("currency_code");
...
for (const row of data ?? []) {
  ...
  console.log(`\n########## ${row.currency_code} — ${items.length} položek agendy ##########`);
  const cats = {};
  for (const s of items) cats[s.tier] = (cats[s.tier] ?? 0) + 1;
  console.log("tiery:", JSON.stringify(cats));
