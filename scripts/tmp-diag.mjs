...
// perEventBase = příspěvek BEZ váhy kategorie (rule.w se aplikuje až při kombinaci) — potřeba
// pro Variantu G, aby šlo spočítat průměrnou SÍLU signálu na kategorii nezávisle na váze.
function collectContributions(currencyCode, calendarEvents, now) {
  ...
  for (const ev of calendarEvents) {
    ...
    const perEventBase = dir * surpriseStrength(ev) * recency(daysAgo) * relevance.factor;
    contributions.push({ cat: rule.cat, w: rule.w, perEventBase });
  }
...
function variantA(contributions, confidence) {
  const totalSigned = contributions.reduce((s, c) => s + c.w * c.perEventBase, 0);
  const rawScore = clamp(totalSigned, -10, 10);
  return Math.round(clamp((rawScore / 2) * confidence, -5, 5) * 100) / 100;
}

// ── VARIANTA G: uvnitř kategorie průměr ZE VŠECH eventů (nic se nezahazuje ani neupřednostňuje
// podle data/velikosti), ale "objem důkazů" na kategorii se zastropuje na CONFIDENCE_CAP
// "ekvivalentních eventů" TOHO SAMÉHO pozorovaného průměru. Když je eventů v kategorii méně
// než CONFIDENCE_CAP, výsledek je IDENTICKÝ současné produkci (matematická identita níže).
function variantG(contributions, confidence, confidenceCap) {
  const byCat = {};
  for (const c of contributions) {
    byCat[c.cat] ??= { w: c.w, sum: 0, count: 0 };
    byCat[c.cat].sum += c.perEventBase;
    byCat[c.cat].count++;
  }
  let rawSum = 0;
  for (const cat of Object.keys(byCat)) {
    const { w, sum, count } = byCat[cat];
    const meanBase = sum / count;
    const cappedCount = Math.min(count, confidenceCap);
    rawSum += meanBase * w * cappedCount;
  }
...
for (const cap of [6, 8, 10]) {
  console.log(`\n\n================ VARIANTA G, CONFIDENCE_CAP=${cap} ================`);

  console.log("\n═══ Historická validace vs. reálná CB Policy kotva ═══");
  for (const ccy of FOCUS) {
    console.log(`\n--- ${ccy} ---`);
    console.log("dní zpět | CB Policy (kotva)                          | A       | G");
    for (const daysAgo of AS_OF_DAYS_AGO) {
      ...
      const g = variantG(contributions, confidence, cap);
      console.log(`${String(daysAgo).padStart(4)} (${asOf.toISOString().slice(0, 10)}) | ${cb.policyLabel.padEnd(43)} | ${a.toFixed(2).padStart(6)} | ${g.toFixed(2).padStart(6)}`);
    }
  }
  ...
  for (const ccy of SCORED) {
    ...
    const g = variantG(contributions, confidence, cap);
    gVals.push(g);
    console.log(`${ccy}: G=${g.toFixed(2)}`);
  }
  function stdev(arr) {
    const m = arr.reduce((s, v) => s + v, 0) / arr.length;
    return Math.sqrt(arr.reduce((s, v) => s + (v - m) ** 2, 0) / arr.length);
  }
  console.log(`sd(G)=${stdev(gVals).toFixed(2)}, min=${Math.min(...gVals)}, max=${Math.max(...gVals)}`);
}
