...
// ── VARIANTA H: JEDEN globální tlumící faktor na CELÝ součet (ne po kategoriích) — nemůže
// nikdy otočit znaménko, protože násobí celý vektor stejnou kladnou konstantou. Aktivuje se
// jen když celkový počet eventů překročí REFERENCE_COUNT (kalibrace na typický objem, pro
// který byl ±10 strop původně navržen); pod tou hranicí je H identická se současnou produkcí.
function variantH(contributions, confidence, referenceCount) {
  const totalSigned = contributions.reduce((s, v) => s + v, 0);
  const n = contributions.length;
  const dampFactor = n > 0 ? Math.min(1, referenceCount / n) : 1;
  const dampedSum = totalSigned * dampFactor;
  const rawScore = clamp(dampedSum, -10, 10);
  return Math.round(clamp((rawScore / 2) * confidence, -5, 5) * 100) / 100;
}
...
for (const ref of [40, 60, 80]) {
  console.log(`\n\n================ VARIANTA H, REFERENCE_COUNT=${ref} ================`);

  console.log("\n═══ Historická validace vs. reálná CB Policy kotva ═══");
  for (const ccy of FOCUS) {
    console.log(`\n--- ${ccy} ---`);
    console.log("dní zpět | CB Policy (kotva)                          | A       | H       | N eventů");
    for (const daysAgo of AS_OF_DAYS_AGO) {
      ...
      const h = variantH(contributions, confidence, ref);
      console.log(`${String(daysAgo).padStart(4)} (${asOf.toISOString().slice(0, 10)}) | ${cb.policyLabel.padEnd(43)} | ${a.toFixed(2).padStart(6)} | ${h.toFixed(2).padStart(6)} | ${contributions.length}`);
    }
  }
  ...
  for (const ccy of SCORED) {
    ...
    const h = variantH(contributions, confidence, ref);
    hVals.push(h);
    console.log(`${ccy}: H=${h.toFixed(2)} (n=${contributions.length})`);
  }
  function stdev(arr) {
    const m = arr.reduce((s, v) => s + v, 0) / arr.length;
    return Math.sqrt(arr.reduce((s, v) => s + (v - m) ** 2, 0) / arr.length);
  }
  console.log(`sd(H)=${stdev(hVals).toFixed(2)}, min=${Math.min(...hVals)}, max=${Math.max(...hVals)}`);
}
